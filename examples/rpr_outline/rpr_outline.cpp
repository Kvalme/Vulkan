...
public:
    ...
    struct Texture {
        ...
        VkImageView view;
    } color_aov, depth_aov, shape_id_aov;
	...
	rpr_framebuffer shape_id_framebuffer_ = nullptr;
    ...
    ~VulkanExample()
    {
        ...
        rprObjectDelete(shape_id_framebuffer_);
		...
		destroyTextureImage(shape_id_aov);
	...
	void rebuildCommandBuffer(std::uint32_t const index)
    {
        ...
        vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, outputPipelineLayout, 0, 1, &outputDescriptorSet, 0, nullptr);
		...
		if (shade_index_ != 0)
		{
			vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, copyDepthPipelineLayout, 0, 1, &copyDepthDescriptorSet, 0, nullptr);
			...
			vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, outlinePipelineLayout, 0, 1, &outlineDescriptorSet, 0, nullptr);
			...
			outlineConstants.transform = cameraController.getProjection() * cameraController.getView();
			outlineConstants.color = glm::vec4(0.2f, 0.2f, 0.9f, 1.0f);
			outlineConstants.tex_coord_offset = glm::vec2(1.0 / width, 1.0 / height);

			vkCmdPushConstants(cmdBuffer, outlinePipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(outlineConstants), &outlineConstants);
    ...
    void setupLayouts()
    {
...

		{
			...
			VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pPipelineLayoutCreateInfo, nullptr, &outlinePipelineLayout));
		}
    }

    void setupDescriptorSet()
    {
		constexpr size_t descriptorSetCount = 3;

		VkDescriptorSetLayout descriptorSetLayouts[descriptorSetCount] = { descriptorSetLayout, descriptorSetLayout, descriptorSetLayout };
		...
		outputDescriptorSet = descriptorSets[0];
		copyDepthDescriptorSet = descriptorSets[1];
		outlineDescriptorSet = descriptorSets[2];
		...
		VkDescriptorImageInfo shapeIdAovDescriptor;
		shapeIdAovDescriptor.imageView = shape_id_aov.view;
		shapeIdAovDescriptor.sampler = shape_id_aov.sampler;
		shapeIdAovDescriptor.imageLayout = shape_id_aov.imageLayout;

		VkWriteDescriptorSet writeDescriptorSets[descriptorSetCount] =
		{
			...
			vks::initializers::writeDescriptorSet(
				...
				VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
				0,
				&shapeIdAovDescriptor
			)
		};

		vkUpdateDescriptorSets(device, (uint32_t)descriptorSetCount, writeDescriptorSets, 0, nullptr);
    }
    ...
    {
		{
			...
			shaderStages[0] = loadShader(getAssetPath() + "shaders/rpr_outline/fullscreen_quad.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
			shaderStages[1] = loadShader(getAssetPath() + "shaders/rpr_outline/output.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
...

		{
			...
			shaderStages[0] = loadShader(getAssetPath() + "shaders/rpr_outline/fullscreen_quad.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
			shaderStages[1] = loadShader(getAssetPath() + "shaders/rpr_outline/copy_depth.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
...

		{
			...
			shaderStages[0] = loadShader(getAssetPath() + "shaders/rpr_outline/outline.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
			shaderStages[1] = loadShader(getAssetPath() + "shaders/rpr_outline/outline.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    void initAovs()
    {
...

		{
			...
			CHECK_RPR(rprContextCreateFrameBuffer(context_, fmt, &desc, &shape_id_framebuffer_));
			CHECK_RPR(rprContextSetAOV(context_, RPR_AOV_OBJECT_ID, shape_id_framebuffer_));
		}
    ...
    void setupTextures()
    {
		...
		{
			CHECK_RPR(rprFrameBufferGetInfo(shape_id_framebuffer_, RPR_VK_IMAGE_OBJECT, sizeof(VkImage), &shape_id_aov.image, 0));
			...
			sampler.anisotropyEnable = VK_FALSE;
			sampler.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
			VK_CHECK_RESULT(vkCreateSampler(device, &sampler, nullptr, &shape_id_aov.sampler));
			...
			view.subresourceRange.layerCount = 1;
			view.subresourceRange.levelCount = 1;
			view.image = shape_id_aov.image;
			VK_CHECK_RESULT(vkCreateImageView(device, &view, nullptr, &shape_id_aov.view));

			shape_id_aov.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		}
    }
    ...
    virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay)
    {
        if (overlay->header("Settings"))
        {
			...
			overlay->comboBox("Outline ID", &shade_index_, {"None", "1", "2", "3", "4", "5"});
        }
    }
};
